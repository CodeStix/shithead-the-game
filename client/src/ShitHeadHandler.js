window.ShitHeadHandler = class ShitHeadHandler extends (
    ...
    create() {
        ...
        this.turnText = this.add.text(
            0.25 * this.game.config.width,
            0.35 * this.game.config.height,
            "Waiting...",
            turnTextStyle
        );
        this.turnText.setDepth(100000000);

        const gameModeButtonStyle = {
            backgroundColor: "#721",
            padding: 6,
            fontSize: 12,
        ...
        this.gameModeButton = this.add.text(
            0.3 * this.game.config.width,
            0.6 * this.game.config.height,
            "Gamemode: Normal",
            gameModeButtonStyle
        );
        this.gameModeButton.setInteractive();
        this.gameModeButton.on("pointerdown", () => {
            if (this.gameModeButton.text === "Gamemode: Normal") this.gameModeButton.text = "Gamemode: NO U";
            else if (this.gameModeButton.text === "Gamemode: NO U") this.gameModeButton.text = "Gamemode: One 10";
            else if (this.gameModeButton.text === "Gamemode: One 10") this.gameModeButton.text = "Gamemode: Instafinal";
            ...
            else if (this.gameModeButton.text === "Gamemode: Trashmode") this.gameModeButton.text = "Gamemode: Normal";
        });
    ...
    setTurn(playerName) {
        if (this.playerAtTurn) this.onTurnWillEnd(this.playerAtTurn);
    ...
    createStacksForPlayer(player) {
        if (this.players.length === 1) {
            // create inventories for the local/first player
            player.playerNameText = this.add.text(
                0.25 * this.game.config.width,
                0.72 * this.game.config.height,
                "YOU",
                { fixedWidth: 0.5 * this.game.config.width, align: "center", fontSize: 16, fontFamily: "Pacifico" }
            );
            ...
            // create inventories for the second player
            player.playerNameText = this.add.text(
                0.02 * this.game.config.width,
                0.62 * this.game.config.height,
                player.name,
                { fixedWidth: 0.5 * this.game.config.width, align: "left", fontSize: 16, fontFamily: "Pacifico" }
            );
            ...
            // create inventories for the third player
            player.playerNameText = this.add.text(
                0.25 * this.game.config.width,
                0.13 * this.game.config.height,
                player.name,
                { fixedWidth: 0.5 * this.game.config.width, align: "center", fontSize: 16, fontFamily: "Pacifico" }
            );
            ...
            // create inventories for the fourth player
            player.playerNameText = this.add.text(
                0.5 * this.game.config.width,
                0.62 * this.game.config.height,
                player.name,
                { fixedWidth: 0.48 * this.game.config.width, align: "right", fontSize: 16, fontFamily: "Pacifico" }
            );
        ...
        if (newState === "startOfGame") {
            ...
            this.players.forEach((player) => {
                ...
                player.finalStack1.onAddingCardToTop = (newCard) => {
                    if (player.finalStack1.containingCards.length === 1) newCard.flipCard(true); // flip the card if it is the second on the stack
                };
                player.finalStack2.onAddingCardToTop = (newCard) => {
                    if (player.finalStack2.containingCards.length === 1) newCard.flipCard(true); // flip the card if it is the second on the stack
                };
                player.finalStack3.onAddingCardToTop = (newCard) => {
                    if (player.finalStack3.containingCards.length === 1) newCard.flipCard(true); // flip the card if it is the second on the stack
                };

                // it should only be switched if the card came from the players inventory
                var switchTopCardCondition = (newTopCard, _oldTopCard) =>
                    newTopCard.snappedToStack.stackName === "inventory" &&
                    newTopCard.snappedToStack.stackOwner === player;
            ...
            if (this.isDealer()) {
                ...
                var cards = [];
                for (let i = 0; i < 52; i++) cards.push(Math.floor(i / 13) + ":" + ((i % 13) + 1));
                cards.push("4:0");
                cards.push("4:0");

                console.log("gamemode", this.gameModeButton.text);
                if (this.gameModeButton.text === "Gamemode: One 10") {
                    // add 4 extra 10s
                    cards = cards.filter((card) => !card.endsWith(":10"));
                    cards.push("0:10");
                } else if (this.gameModeButton.text === "Gamemode: NO U") {
                    // add an extra 4 7s and 4 aces.
                    for (let j = 0; j < 4; j++) {
                        cards.push((j % 4) + ":7");
                        cards.push((j % 4) + ":1");
                    }
                } else if (this.gameModeButton.text === "Gamemode: Instafinal") {
                    // start the finale immediately
                    cards.splice(this.players.length * 9);
                } else if (this.gameModeButton.text === "Gamemode: Jokerparty") {
                    // add 6 extra jokers
                    for (let j = 0; j < 12; j++) cards.push("4:0");
                } else if (this.gameModeButton.text === "Gamemode: Trashmode") {
                    // add 4 extra 10s
                    cards = [];
                    for (let i = 0; i < 52; i++) {
                        var value = (i % 13) + 1,
                            type = Math.floor(i / 13);

                        if (value > 2 && value < 10) cards.push(type + ":" + value);
                    }

                    for (let j = 0; j < 4; j++) cards.push((j % 4) + ":3");
                }
            ...
            throwStack.onCardWantsToGoHere = (newCard) => {
                ...
                if (this.getPlayerStage(this.localPlayer) === 2) {
                    ...
                    if (!mayThrow) {
                        if (this.previouslyThrownValueThisRound === null) newCard.flipCard(true);
            ...
            throwStack.onAddedCardToTop = (newCard) => {
                ...
                if (playerStage === 3) {
                    ...
                    if (!this.playerWon) this.playerWon = this.playerAtTurn;
                ...
                if (this.isAtTurn()) {
                    // only the player at turn should run the following code
    ...
    takeThrowStack() {
        ...
        this.server.send(
            "broadcast deal " +
                throwStack.containingCards.length +
                " throw " +
                stackToString(this.localPlayer.inventory) +
                "|broadcastall turn " +
                this.getNextTurnPlayer().name
        );
    ...
    takeMinCards(amount = 3) {
        ...
        if (playerInvCards < amount) {
            this.server.send(
                "broadcast deal " + (amount - playerInvCards) + " take " + stackToString(this.localPlayer.inventory)
            );
    ...
    getPlayerStage(
        player // normal(0), final(1), hiddenfinal(2) or done(3)
    ) {
        if (!player.inventory.isEmpty() || !this.getStack("take").isEmpty()) return 0;

        var finalCount =
            player.finalStack1.containingCards.length +
            player.finalStack2.containingCards.length +
            player.finalStack3.containingCards.length;

        if (finalCount > 3) return 1;
        else if (finalCount > 0) return 2;
        else return 3;
    }
    ...
    countCardValues(player, value) {
        ...
        if (playerStage === 0) {
            ...
            return (
                (getCardValue(player.finalStack1.containingCards[1]) === value ? 1 : 0) +
            ...
            return (
                (getCardValue(player.finalStack1.containingCards[0]) === value ? 1 : 0) +
    ...
    onTurnWillEnd(playerTurnEnd) {
        ...
        if (this.isAtTurn()) this.takeMinCards();
    ...
    canPlay(card, player) {
        ...
        if (underlayingValue === 1)
            // the ace has a value of 14, not 1
            underlayingValue = 14;

        const playerStage = this.getPlayerStage(player);
        if (playerStage === 0) {
            // if normal inventory move
            if (card.snappedToStack !== player.inventory) return false;
        } else if (playerStage === 1) {
            // if final stack move
            if (card.snappedToStack.containingCards.length !== 2) return false;
        } else if (playerStage === 2) {
            // if final hidden stack move
            if (card.snappedToStack.containingCards.length !== 1) return false;
        }

        if (
            throwStack.getTopCard() &&
            throwStack.getTopCard().cardValue === card.cardValue &&
            throwStack.getSameValueDepthFromTop() + this.countCardValues(player, card.cardValue) >=
